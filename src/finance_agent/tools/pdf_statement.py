...
def extract_text_from_pdf(pdf_source) -> str:
    ...
    - raw bytes (bytes / bytearray) e.g. from FastAPI UploadFile.read()

    When bytes are provided, we wrap them in io.BytesIO because pdfplumber/pdfminer
    expects a file-like object that supports .seek().
    """
    import io
    from pathlib import Path

    if isinstance(pdf_source, (bytes, bytearray)):
        opener = io.BytesIO(pdf_source)
    else:
        opener = str(Path(pdf_source))
...
# --- Public API -------------------------------------------------------------
def parse_statement_transactions_pdf(pdf_source) -> List[StatementTx]:
    """Main entrypoint used by the CLI/ingestion service."""

    raw = extract_text_from_pdf(pdf_source)
    if not raw.strip():
        return []
